...
function resetGame() {
 ...
 if (hardMode) {
   noteElement.innerHTML = `Click the joystick to start!
       <h2>Hard Mode: ON</h2><bR><p>Hard mode, Avoid black holes. Back to easy mode? Press the Switch Button</p>`;
 } else {
   noteElement.innerHTML = `Click the joystick to start!
...
function main(timestamp) {
 ...
 try {
   ...
   if (error.message == "The ball fell into a hole") {
     noteElement.innerHTML = `A ball fell into a black hole! Press Reset to restart the game.
       <p>
